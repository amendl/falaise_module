...
dpp::chain_module::process_status EnergyHistogramModule::process (datatools::things & event) 
{
 ...
 //  auto result_tensor = .get_tensor()
	...
	auto output = model->operator()(c);
	auto result = output.get_data<float>();
	// auto arr = static_cast<float*>(TF_TensorData(model->operator()(c).get_tensor().get()));
	// std::cout<<result[0]<<" "<<result[1]<<" "<<result[2]<<" "<<result[3]<<std::endl;
	// std::cout<<my_simple_argmax(result)<<std::endl;
	// std::cin.get();
...
{	
	...
	// Save all data
	std::ofstream myfile;
	myfile.open("results_0.txt",std::ios::out);
	std::cout<<classification[0]<<","<<classification[1]<<","<<classification[2]<<","<<classification[3]<<std::endl;
...
// std::cout<<std::endl<<std::endl<<"Before return "<<cache_9_113.size()<<std::endl<<std::endl;
	return cppflow::tensor(this->cache_9_113, {1,9,113});
}
